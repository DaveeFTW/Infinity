...
MYFILE *my_fopen(const unsigned char *data, size_t size)
{
...
int my_fseek(MYFILE *f, long int offset,int origin)
{
  if (origin == SEEK_SET)
  {
    f->pos = offset;
...
long int my_ftell(MYFILE *f)
{
  return f->pos;
}

size_t my_fread ( void * ptr, size_t size, size_t count, MYFILE * f )
{
  size_t copylen = size*count;
  if (f->pos+copylen > f->size)
  {
    copylen = f->size-f->pos;
  }

  memcpy(ptr, f->data+f->pos, copylen);
  f->pos+= copylen;
  return count;
}
...
unsigned long* my_intraFontGetTable(MYFILE *file, unsigned long n_elements, unsigned long bp_element) {
  ...
  if (my_fread(raw_table, len_table*sizeof(unsigned char), 1, file) != 1) {
    free(raw_table);
    return NULL;
  }
...
intraFont* intraFontLoadMemory(const unsigned char *data, size_t size, unsigned int options) {
    unsigned long i,j;
  static Glyph bw_glyph                         = { 0, 0, 16, 18, 0, 15, PGF_BMP_H_ROWS, 0, 64, 0 };
  static Glyph bw_shadowGlyph                   = { 0, 0,  8, 10, 0,  5, PGF_BMP_H_ROWS, 0, 64, 0 };
  static const unsigned short bw_charmap_compr[]= { 0x00a4,  1, 0x00a7,  2, 0x00b0,  2, 0x00b7,  1, 0x00d7,  1, 0x00e0,  2, 0x00e8,  3, 0x00ec,  2,
  ...
  //open pgf file and get file size
    MYFILE *file = my_fopen(data, size); /* read from the file in binary mode */
  if (!file) return NULL;
  my_fseek(file, 0, SEEK_END);
    unsigned long filesize = my_ftell(file);
  my_fseek(file, 0, SEEK_SET);
  
  //read pgf header
  static PGF_Header header;
  if (my_fread(&header, sizeof(PGF_Header), 1, file) != 1) {
    my_fclose(file);
    return NULL;
  }
  ...
  if ((options & INTRAFONT_CACHE_ASCII) ) font->options -= INTRAFONT_CACHE_ASCII; //pre-cached texture enabled at the end of font-load
    ...
    font->isRotated = 0;
  font->altFont = NULL;            //no alternative font
  font->filename = (char*)malloc((strlen("memory")+1)*sizeof(char));
  font->texture = (unsigned char*)memalign(16,font->texWidth*font->texHeight>>1);
  if (!font->filename || !font->texture) {
    my_fclose(file);
    intraFontUnload(font);
  ...
  strcpy(font->filename,"memory");
  
  //read in file (tables / fontdata) and populate font structure
  if (font->fileType == FILETYPE_PGF) {

    //read advance table
    my_fseek(file, header.header_len+(header.table1_len+header.table2_len+header.table3_len)*8, SEEK_SET);
    signed long *advancemap = (signed long*)malloc(header.advance_len*sizeof(signed long)*2);
    if (!advancemap) {
      my_fclose(file);
      intraFontUnload(font);
    ...
    if (my_fread(advancemap, header.advance_len*sizeof(signed long)*2, 1, file) != 1) {
      free(advancemap);
      my_fclose(file);
      intraFontUnload(font);
      return NULL;
    ...
    //read shadowmap
    unsigned long *ucs_shadowmap = my_intraFontGetTable(file, header.shadowmap_len, header.shadowmap_bpe);
    if (ucs_shadowmap == NULL) {
      free(advancemap);  
      my_fclose(file);
      intraFontUnload(font);
    ...
    //version 6.3 charmap compression
    if (header.revision == 3) {
      if (my_fread(font->charmap_compr, font->charmap_compr_len*sizeof(unsigned short)*2, 1, file) != 1) {
        free(advancemap);    
        free(ucs_shadowmap);
        my_fclose(file);
        intraFontUnload(font);
    ...
    //read charmap
    if (header.charmap_bpe == 16) { //read directly from file...
      if (my_fread(font->charmap, header.charmap_len*sizeof(unsigned short), 1, file) != 1) {
        free(advancemap);  
        free(ucs_shadowmap);
        my_fclose(file);
        intraFontUnload(font);
      ...
      unsigned long *id_charmap = my_intraFontGetTable(file, header.charmap_len, header.charmap_bpe);
      if (id_charmap == NULL) {
        free(advancemap);  
        free(ucs_shadowmap);
        my_fclose(file);
    ...
    //read charptr
    unsigned long *charptr = my_intraFontGetTable(file, header.charptr_len, header.charptr_bpe);
    if (charptr == NULL) {
      free(advancemap);  
      free(ucs_shadowmap);
      my_fclose(file);
    ...
    //read raw fontdata
    unsigned long start_fontdata = my_ftell(file);
    unsigned long len_fontdata = filesize-start_fontdata;
    font->fontdata = (unsigned char*)malloc(len_fontdata*sizeof(unsigned char));
    if (font->fontdata == NULL) {
      free(advancemap);  
    ...
    if (my_fread(font->fontdata, len_fontdata*sizeof(unsigned char), 1, file) != 1) {
      free(advancemap);  
      free(ucs_shadowmap);
      free(charptr);
      my_fclose(file);
    ...
    if (my_fread(font->fontdata, filesize*sizeof(unsigned char), 1, file) != 1) {
      my_fclose(file);
      intraFontUnload(font);
      return NULL;
    }
